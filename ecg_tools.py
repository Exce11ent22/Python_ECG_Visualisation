...
def get_p_info(signals):
    ...
    for i in range(valid_index, len(signals) + valid_index):
        if signals.ECG_P_Peaks[i] == 1:
            result[0] = str(round(signals.ECG_Clean[i], 2)) + " mV"
            break
    return result
...
def get_pq_info(signals):
    ...
    for i in range(valid_index, len(signals) + valid_index):
        ...
        if signals.ECG_Q_Peaks[i] == 1:
            ...
            break
    return str(round((end - start), 2)) + " ms"
...
def get_info(signals):
    info = [
        ["P", get_p_info(signals)[0]],
        ["Q", get_q_info(signals)[0]],
        ["R", get_r_info(signals)[0]],
        ["S", get_s_info(signals)[0]],
        ["T", get_t_info(signals)[0]],
        ["P", "48 ms"],
        ["Q", "9 ms"],
        ["R", "36 ms"],
        ["S", "31 ms"],
